...
$query3 = "SELECT DISTINCT `contract` FROM `candidates`";
$query4 = "SELECT DISTINCT `city` FROM `candidates`";
$query5 = "SELECT DISTINCT `state` FROM `candidates`";
$query6 = "SELECT DISTINCT `post` FROM `candidates`";
$query7 = "SELECT DISTINCT `status` FROM `candidates`";
...
if (isset($_POST['submit'])) {
    ...
    $notSubmit = false;
    $query2 = "SELECT * FROM `candidates` WHERE `contract` = '".$contract."' OR `city` = '".$city."' OR `state` = '".$state."' OR `post` = '".$post."' OR `status` = '".$status."'";

    $result2 = mysqli_query($link,$query2);
}
else{
    $notSubmit = true;
    $query1 = "SELECT * FROM `candidates` WHERE 1";
    ...
    <div class="main-wrapper">
	   ...
	   <div class="row">
		<div class="col-sm-9">
            <?php
                if($notSubmit){
                    echo "<h2 class='page-header'><strong>".mysqli_num_rows($result1)."</strong> candidates matching your query</h2>";
                }
                else{
                    echo "<h2 class='page-header'><strong>".mysqli_num_rows($result2)."</strong> candidates matching your query</h2>";
                }
            ?>
...
<div class="candidates-list">
    ...
    if($notSubmit){
        while ($row = mysqli_fetch_assoc($result1)) {
            echo "<div class='candidates-list-item'>
                    <div class='candidates-list-item-heading'>
               
                        <div class='candidates-list-item-title'>
                            <h2><a href='resume.php'>".$row['fname']." ".$row['lname']."</a></h2>
                            <h3>".$row['post']."</h3>
                        </div><!-- /.candidates-list-item-title -->
                    </div><!-- /.candidates-list-item-heading -->

                    <div class='candidates-list-item-location'>
                        <i class='fa fa-map-marker'></i>".$row['city'].", ".$row['state']." 
                        <h5>".$row['status']."</h5>
                    </div><!-- /.candidates-list-item-location -->

                    <div class='candidates-list-item-profile'>
                        <span class='candidates-list-item-profile-count'>".$row['profile_completed']."% completed</span>
                        <span class='candidates-list-item-profile-bar'><span style='width:".$row['profile_completed']."%; '></span></span>
...
<div class="col-sm-3">
    ...
    <form method="post" action="candidates.php">
        ...
        <?php 
            ...
            while($row = mysqli_fetch_assoc($result3)){
                echo "<div class='checkbox'>
                        <label><input type='radio' name='contract' value='".$row['contract']."'> ".$row['contract']."</label>
                      </div><!-- /.checkbox -->";
            }
        ...
        ?>
        <button type="submit" name="submit" class="btn btn-secondary btn-block"><i class="fa fa-refresh"></i> Reset Filter</button>
    </form>
</div><!-- /.filter-stacked -->
...
<!-- Mirrored from preview.byaviators.com/template/profession/candidates.php by HTTrack Website Copier/3.x [XR&CO'2014], Sun, 19 Aug 2018 11:09:12 GMT -->
</html>
